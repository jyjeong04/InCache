...
extern double
    AddGPUBurden[58]; //->initial in handshaking. fix rLen to 1024*1024
extern double
    AddCPUBurden[58]; //->initial in handshaking. fix rLen to 1024*1024
...
extern double cpuburden[100000];
extern double gpuburden[100000];
...
double inline getAddBurden_Copy(const int *Flag_CPU_GPU, double size) {
  if ((*Flag_CPU_GPU))
    ...
    return AddCPUBurden_Copy / base * size;
}

double inline getAddBurden_Read(const int *Flag_CPU_GPU, double size) {
  if ((*Flag_CPU_GPU))
    return AddGPUBurden_Read / base * size;
  else
    return AddCPUBurden_Read / base * size;
}
double inline getAddBurden_Write(const int *Flag_CPU_GPU, double size) {
  if ((*Flag_CPU_GPU))
    return AddGPUBurden_Write / base * size;
  else
    return AddCPUBurden_Write / base * size;
}
...
int Kernelscheduler(int size, int kid, int *Flag_CPU_GPU, double *burden,
  ...
  if (global_KernelSchedule) {
    ...
    if ((CPUBurden + getAddCPUBurden(kid, size)) <
        (GPUBurden + getAddGPUBurden(kid, size))) {
      CPU_GPU = 0;
...
int cl_readbufferscheduler(int size, int *Flag_CPU_GPU, double *burden,
                           int _CPU_GPU) {
  int CPU_GPU;
...
int cl_writebufferscheduler(int size, int *Flag_CPU_GPU, double *burden,
                            int _CPU_GPU) {
  int CPU_GPU;
...
int cl_copyBufferscheduler(int size, int *Flag_CPU_GPU, double *burden,
                           int _CPU_GPU) {
  int CPU_GPU;
